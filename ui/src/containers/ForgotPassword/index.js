      ...
      <div>
        <form className="col-md-12" onSubmit={this.onSubmit}>
          ...
          <div className={styles.buttonGroup}>
            <button type="submit" className="btn btn-primary">{textCN.getSiteTextById('homeSendResetInstructions')}</button>
          </div>
        </form>
      </div>
    );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <FullPageBackground>
          <h3 style={{ marginBottom: '50px' }}>{textCN.getSiteTextById('homeForgottenPassword')}</h3>
          { stateView }
        </FullPageBackground>
      </div>
    );
