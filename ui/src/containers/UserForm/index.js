...
const renderVerified = (model) => {
  ...
  return (
    <div className="form-group">
      <label htmlFor={verifiedId} className="control-label">{textCN.getSiteTextById('saUserVerified')}</label>
      <span id={verifiedId}>
        {model.get('verified')
      ...
      <div id={passwordId}>
        {model.get('verified') ? (
          <Checkbox
            ...
            label={textCN.getSiteTextById('saUserChangePassword')}
            onChange={onCheck} />
        ) : (
...
const renderPassword = (password, setPassword, passwordGroupClasses, passwordErrors) => {
  ...
  return (
    <div className={passwordGroupClasses}>
      <label htmlFor={passwordId} className="control-label">{textCN.getSiteTextById('saUserNewPassword')}</label>
      <input
        id={passwordId}
        className="form-control"
        placeholder={textCN.getSiteTextById('saUserNewPassword')}
        autoComplete="false"
        type="password"
        value={password}
...
const renderPasswordConfirmation = (passwordConfirmation, setPasswordConfirmation, passwordGroupClasses) => {
  ...
  return (
    <div className={passwordGroupClasses}>
      <label htmlFor={confirmationId} className="control-label">{textCN.getSiteTextById('saUserConfirmPassword')}</label>
      <input
        id={confirmationId}
        className="form-control"
        placeholder={textCN.getSiteTextById('saUserConfirmPassword')}
        autoComplete="false"
        type="password"
        ...
        {passwordInputsVisible && (
          <div className="form-group">
            ...
            <button
              ...
              <i className="ion ion-checkmark" /><span> {textCN.getSiteTextById('saUserSetPassword')}</span>
            </button>
          </div>
        )}
      </div>
