    ...
    return (
      !canViewSettings ? <noscript /> : (
        <CollapsibleNav
          ...
          routeParams={{ organisationId }}
          label= {textCN.getSiteTextById('saTitle')}
          icon={'ion ion-gear-a'} >
          {canViewStores && this.renderSettingsLink('stores',  textCN.getSiteTextById('setStores')   )}
          {canViewUsers && this.renderSettingsLink('users', textCN.getSiteTextById('saUsers')   )}
          {canViewOrganisations && this.renderSettingsLink('suborgs', textCN.getSiteTextById('saOrganisation')   )}
          {canViewClients && this.renderSettingsLink('clients', textCN.getSiteTextById('setClients')   )}
          {canViewRoles && this.renderSettingsLink('roles', textCN.getSiteTextById('setRoles')   )}
        </CollapsibleNav>
      )
    );
  }
  ...
  renderPeople = () => {
    ...
    return !canViewPeople ? <noscript /> : (
      <CollapsibleNav
        ...
        {this.renderLink(activeClass, 'organisation.people.manage', textCN.getSiteTextById('orgManage') )}
        {this.renderLink(activeClass, 'organisation.people.reconcile', textCN.getSiteTextById('orgReconcile') )}
      </CollapsibleNav>
    );
  }
      ...
      <div className={`col-sm-3 col-md-2 ${styles.sidenav}`} >
        ...
        <ul className={`nav ${styles.navSidenav}`}>
          <CollapsibleNav
            ...
            routeParams={{ organisationId }}
            label={textCN.getSiteTextById('orgData')}
            manuallyExpanded={!!groups['organisation.data']}
            icon={'ion ion-ionic'} >
            {this.renderLink(activeClass, 'organisation.data.dashboards', textCN.getSiteTextById('orgDashboards')  )}
            {this.renderLink(activeClass, 'organisation.data.visualise', textCN.getSiteTextById('orgVisualise')  )}
            {this.renderLink(activeClass, 'organisation.data.source', textCN.getSiteTextById('orgSource')  )}
            {this.renderLink(activeClass, 'organisation.data.statementForwarding', textCN.getSiteTextById('orgStatementForwarding')  )}
          </CollapsibleNav>
